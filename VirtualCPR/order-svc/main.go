...
func getUser(userID int) (User, error) {
    ...
    url := fmt.Sprintf("http://user-svc:3001/api/users/%d", userID)
...
func processPayment(order Order) (PaymentResponse, error) {
    ...
    client := &http.Client{Timeout: 5 * time.Second}
    resp, err := client.Post("http://payment-svc:3003/api/payments", "application/json", bytes.NewBuffer(payload))
    if err != nil {
        return PaymentResponse{}, err
    }
    defer resp.Body.Close()
...
func sendNotification(orderID int, paymentStatus string, amount int, userEmail string) error {
    ...
    client := &http.Client{Timeout: 5 * time.Second}
    resp, err := client.Post("http://notification-svc:3004/api/notifications", "application/json", bytes.NewBuffer(payload)) 
    if err != nil {
        return err
    }
    defer resp.Body.Close()
